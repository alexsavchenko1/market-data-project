...
def _as_mapping(
    ...
    if not isinstance(value, dict):
        raise MarketDataResponseError(f"Поле '{field_name}' должно быть объектом")
...
def _as_list(
    ...
    if not isinstance(value, list):
        raise MarketDataResponseError(f"Поле '{field_name}' должно быть списком")
...
def parse_yahoo_history(
    ...
    if chart_error is not None:
        raise MarketDataResponseError(f"Yahoo Finance вернул ошибку: {chart_error}")

    results = _as_list(chart.get("result"), "chart.result")

    if not results:
        raise MarketDataResponseError("Yahoo Finance не вернул данные по тикеру")
    ...
    if not isinstance(currency, str) or not currency.strip():
        raise MarketDataResponseError("В ответе отсутствует валюта инструмента")
    ...
    if not adjusted_close_blocks:
        raise MarketDataResponseError("В ответе отсутствуют скорректированные цены")
    ...
    if len(timestamps) != len(prices):
        raise MarketDataResponseError("Количество дат не совпадает с количеством цен")
    ...
    for timestamp_value, price_value in zip(timestamps, prices, strict=True):
        ...
        if isinstance(timestamp_value, bool) or not isinstance(timestamp_value, (int, float)):
            raise MarketDataResponseError("Ответ содержит некорректную временную метку")

        if isinstance(price_value, bool) or not isinstance(price_value, (int, float, str)):
            raise MarketDataResponseError("Ответ содержит некорректную цену")
        ...
        try:
            ...
            raise MarketDataResponseError("Не удалось преобразовать точку истории цен") from error
    ...
    try:
        ...
        raise MarketDataResponseError("Yahoo Finance вернул некорректную историю цен") from error
    ...
    def fetch_history(
        ...
        try:
            ...
            raise MarketDataRequestError(
                f"Не удалось получить данные для {request.ticker.symbol}"
            ) from error

        try:
            payload = cast(object, response.json())
        except ValueError as error:
            raise MarketDataResponseError("Yahoo Finance вернул некорректный JSON") from error
